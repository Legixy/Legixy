// ============================================================================
// OnyxLegal — Database Seed Script
// Creates realistic demo data for development and testing
// Run: npm run db:seed
// ============================================================================

import 'dotenv/config';
import { PrismaClient, Plan, UserRole, ContractStatus, RiskLevel, ClauseType, AnalysisStatus, AnalysisType, NotificationType, TemplateCategory } from '../generated/prisma/client';
import { PrismaPg } from '@prisma/adapter-pg';

// Parse DATABASE_URL — handle both prisma+postgres:// and postgres:// formats
...
async function main() {
  ...
  const contracts = await Promise.all([
    // Contract 1: High risk vendor agreement
    ...
    // Contract 3: Low risk employment
    prisma.contract.upsert({
    ...
    // Contract 4: Critical risk - no review yet
    prisma.contract.upsert({
    ...
    // Contract 5: Signed safe contract
    prisma.contract.upsert({
  ...
  // ── 5. Clauses for Contract 1 (high risk) ────────────────
  ...
  const analysis = await prisma.aIAnalysis.upsert({
    ...
    create: {
      id: 'analysis-001',
      contractId: 'contract-001',
      type: AnalysisType.DEEP_ANALYSIS,
  ...
  await Promise.all([
    ...
    prisma.notification.create({
      data: {
        userId: owner.id,
        type: NotificationType.AI_FIX_READY,
